...
LAYOUT_ALGORITHM = 'gvpr' # ['neato'|'dot'|'twopi'|'circo'|'fdp'|'nop']
REPRESENT_CHANNELS_AS_NODES = 1
DEFAULT_NODE_SIZE = 8.0 # default node size in meters
    ...
    def _update_appearance(self):
        ...
        if self._label is not None:
            if self._label_canvas_item is None:
                self._label_canvas_item = goocanvas.Text(visibility_threshold=0.3,
                                                         font="Sans Serif 22",
...
class Edge(Link):
    def __init__(self, node1, node2, color="black", width_=3.0):
        assert isinstance(node1, Node)
        assert isinstance(node2, Node)
        self.node1 = node1
        self.node2 = node2
        self.canvas_item = goocanvas.Path(line_width=width_, stroke_color=color)
        self.canvas_item.set_data("pyviz-object", self)
    ...
    def update_color(self, color):
        self.canvas_item.set_property("stroke_color", color)
...
class Visualizer(gobject.GObject):
    ...
    def __init__(self):
        ...
        self.edges_group = goocanvas.Group()
    ...
    def create_gui(self):
        ...
        self.canvas.get_root_item().add_child(self.edges_group)
        self.edges_group.set_property("visibility", goocanvas.ITEM_VISIBLE)
...
# WHY THE HELL DID I HAVE TO TAKE THESE AWAY????? FIXME
...
#        graph.layout(LAYOUT_ALGORITHM)
    ...
    def create_edge(self, node, node2, color="black", width=3.0):
        edge = Edge(node, node2, color, width)
        self.edges_group.add_child(edge.canvas_item)
        edge.canvas_item.lower(None)
        edge.update_points()
        return edge
